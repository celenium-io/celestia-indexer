CREATE MATERIALIZED VIEW IF NOT EXISTS block_stats_by_week
WITH (timescaledb.continuous, timescaledb.materialized_only=true) AS
	select 
		time_bucket('1 week'::interval, day.ts) AS ts,
		sum(blobs_size)/(7 * 86400.0) as bps,
		max(bps_max) as bps_max,
		min(bps_min) as bps_min,
		sum(tx_count)/(7 * 86400.0) as tps,
...
SELECT add_continuous_aggregate_policy('block_stats_by_week',
  start_offset => NULL,
...
WHERE NOT (SELECT EXISTS (
    ...
    WHERE user_view_schema = 'public' AND user_view_name = 'block_stats_by_week'
    )
);
