...
WITH (timescaledb.continuous, timescaledb.materialized_only=false) AS
    select 
        ...
        sum(case 
            ...
            when type = 'unbonded' then change 
            else 0 
            end) as flow,
        sum(case when type = 'delegation' then change else 0 end) as delegations,
        sum(case when type = 'unbonded' then -change else 0 end) as unbondings,
        ...
        sum(case when type = 'unbonded' then 1 else 0 end) as unbondings_count
