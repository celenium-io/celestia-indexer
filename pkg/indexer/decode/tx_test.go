...
func TestDecodeCosmosTx_TimeoutHeightOverflow(t *testing.T) {
	rawTx := []byte{
	...
	var tx sdkTx.Tx
	require.NoError(t, tx.Unmarshal(rawTx))
	tx.Body.TimeoutHeight = stdMath.MaxUint64
	patched, err := tx.Marshal()
	require.NoError(t, err)

	var d = NewDecodedTx()
	err = decodeCosmosTx(txDecoder, patched, &d)
	require.NoError(t, err)
	require.Zero(t, d.TimeoutHeight)
}
