...
func NewAPI(cfg config.DataSource) API {
	...
	return API{
		...
		log:       log.With().Str("module", "DeFi Lama api").Logger(),
	}
}
...
func closeWithLogError(stream io.ReadCloser, log zerolog.Logger) {
	if _, err := io.Copy(io.Discard, stream); err != nil {
		log.Err(err).Msg("DeFi Lama api copy GET body response to discard")
	}
	if err := stream.Close(); err != nil {
		log.Err(err).Msg("DeFi Lama api close GET body request")
	}
}
