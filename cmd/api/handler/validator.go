...
type ValidatorHandler struct {
	...
	bondUpdates     storage.IValidatorBondUpdate
...
type validatorsPagination struct {
	...
	Status  storageTypes.ValidatorStatus `query:"status"  validate:"omitempty,validator_status"`
}
...
//	@Param			status	query	types.ValidatorStatus	false	"Validator status"				example(active)
//	@Produce		json
...
func (handler *ValidatorHandler) Count(c echo.Context) error {
	countByStatus, err := handler.validators.CountByStatus(c.Request().Context())
	if err != nil {
		return handleError(c, err, handler.validators)
	}
	...
	return c.JSON(http.StatusOK, responses.ValidatorCount{
		...
		Jailed:   countByStatus.Jailed,
		Active:   countByStatus.Active,
		Inactive: countByStatus.NotActive,
	})
}
...
type bondUpdatesRequest struct {
	Id     uint64 `param:"id"     validate:"required,min=1"`
	Limit  int    `query:"limit"  validate:"omitempty,min=1,max=100"`
	Offset int    `query:"offset" validate:"omitempty,min=0"`
	Sort   string `query:"sort"   validate:"omitempty,oneof=asc desc"`
...
func (req *bondUpdatesRequest) SetDefault() {
	if req.Limit == 0 {
		req.Limit = 10
	}
}
...
// BondUpdates godoc
//
//	@Summary		Get validator's bond updates
...
//	@Produce		json
//	@Success		200	{array}		responses.BondUpdate
//	@Failure		400	{object}	Error
//	@Failure		500	{object}	Error
//	@Router			/validators/{id}/bond_updates [get]
func (handler *ValidatorHandler) BondUpdates(c echo.Context) error {
	req, err := bindAndValidate[bondUpdatesRequest](c)
	if err != nil {
		return badRequestError(c, err)
	}
	req.SetDefault()

	bondUpdates, err := handler.bondUpdates.ListByValidator(c.Request().Context(), req.Id, storage.FilterBondUpdatesListByValidator{
		Limit:  req.Limit,
		Offset: req.Offset,
		Sort:   pgSort(req.Sort),
	})
	...
	response := make([]responses.BondUpdate, len(bondUpdates))
	for i := range bondUpdates {
		response[i] = responses.NewBondUpdate(bondUpdates[i])
	}
	return returnArray(c, response)
}
